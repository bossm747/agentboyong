...
import { intelligentVerification } from './intelligentVerificationService';
...
export class AIService {
  ...
  private async buildIntelligentPrompt(memoryContext: MemoryContext, mode: string, message: string): Promise<string> {
    // Use intelligent verification system instead of basic prompts
    ...
    if (verificationResult.needsVerification) {
      console.log(`🧠 Intelligent verification activated: ${verificationResult.reasoning}`);
      console.log(`📋 Verification checks: ${verificationResult.checks.length}`);
      
      // Return the enhanced prompt with real verification data
      let enhancedPrompt = verificationResult.enhancedPrompt;
      
      // Add memory context
      if (memoryContext.recentConversations.length > 0) {
        enhancedPrompt += "\n\n## RECENT CONVERSATION CONTEXT:\n";
        memoryContext.recentConversations.slice(0, 3).forEach(conv => {
          enhancedPrompt += `- ${conv.role}: ${conv.content.substring(0, 100)}...\n`;
        });
      }
    ...
    // Fallback to regular intelligent prompt for non-technical queries
    return this.buildRegularIntelligentPrompt(memoryContext, mode);
  }

  private buildRegularIntelligentPrompt(memoryContext: MemoryContext, mode: string): string {
    let prompt = `You are Pareng Boyong, an extremely intelligent Filipino AI AGI Super Agent created by InnovateHub PH.

## INTELLIGENCE PRINCIPLES:
1. 🚫 NEVER rely on assumptions or outdated general knowledge
2. ✅ ALWAYS seek current, verified information when possible
3. 🔍 Be specific and factual, avoid vague generalizations
4. 📚 When uncertain, clearly state the need for verification
5. ⚡ Provide actionable, intelligent responses
...
`;
  ...
  async processMessage(sessionId: string, message: string, mode: string = 'default', userId: string = 'default_user'): Promise<AIResponse> {
    ...
    try {
      ...
      // Intelligent Verification System - Makes Pareng Boyong extremely fact-based
      console.log('🧠 Activating intelligent verification system...');
      let contextualSystemPrompt = await this.buildIntelligentPrompt(memoryContext, mode, message);
      
      // Additional Context7 enhancement for complex queries
      try {
        ...
        if (enhancement.shouldUse && enhancement.libraries.length > 0) {
          console.log(`🔍 Intelligent Context7 detected: ${enhancement.libraries.join(', ')}`);
          console.log(`📋 Reason: ${enhancement.reason}`);
          toolsUsed.push('intelligent_context7', 'verification_system');
        }
      } catch (error) {
        console.warn('⚠️ Intelligent Context7 enhancement failed:', error);
      }
